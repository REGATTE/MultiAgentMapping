...
void distributedMapping::publishGlobalMap(){
	...
	for (int i = 0; i < (int)indices.size(); ++i){
		...
		*global_map_keyframes += *transformPointCloud(robots[robot_id].keyframe_cloud_array[pose_6d_tmp.intensity],
			&pose_6d_tmp);
	}
	...
	global_map_msg.header.stamp = robots[robot_id].time_cloud_input_stamp;
...
void distributedMapping::publishLoopClosureConstraint(){
	...
	for(auto it = loop_indexs.begin(); it != loop_indexs.end(); ++it){
		...
		pose = initial_values->at<Pose3>(Symbol('a'+robot_id, index0));
		p.x = pose.translation().x();
		p.y = pose.translation().y();
		p.z = pose.translation().z();
		nodes.points.push_back(p);
		constraints.points.push_back(p);
		pose = initial_values->at<Pose3>(Symbol('a'+robot_id, index1));
		p.x = pose.translation().x();
		p.y = pose.translation().y();
		p.z = pose.translation().z();
		nodes.points.push_back(p);
